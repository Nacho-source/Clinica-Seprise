...
Public Class FormularioPedido
    ...
    Private Sub FormularioPedido_Activated(sender As Object, e As EventArgs) Handles MyBase.Activated
        i = 0
    ...
    Private Sub btnRegI_Click(sender As Object, e As EventArgs) Handles btnRegI.Click
        If btnRegI.Text <> "Cancelar" Then
            If IsNothing(dgvGrillaPedidoI.CurrentRow) Then
                IdDetPedido = dgvGrillaPedidoI.Rows(0).Cells.Item(1).Value
                IdPedido = dgvGrillaPedidoI.Rows(0).Cells.Item(0).Value
    ...
    Private Sub btnRegArt_Click(sender As Object, e As EventArgs) Handles btnRegArt.Click
        ...
        If (Int(txtCantIng.Text)) > lblCantR.Text Then
            ...
            If opc = 6 Then
                If i = 0 Then
                    ...
                    Else
                        ...
                        IdDetPedido = dgvGrillaPedidoM.CurrentRow.Cells.Item(1).Value
                    End If
                    sql = "insert into paquete values ('', " & IdPedido & ", 0, '')"
                    Consulta()
                    MsgBox("Se ha creado el paquete del pedido")
                    sql = "Select max(IdPaq) from Paquete"
                    rs = Consulta()
                    rs.Read()
                    IdPaquete = rs.Item(0)
                End If
                ...
                If opc = 7 Then
                    ...
                    Do While (eoff = False)
                        If i > 0 Then
                            sql = "select Pe.detalle from Pendientes Pe, Paquete Pa where Pe.IdPaq=" & IdPaquete
                            rs2 = Consulta()
                            rs2.Read()
                        End If
                        If (rs.Read = True) Then
                            If rs.Item(2) > 0 Then
                                If i = 0 Then
                                    sql = "insert into Pendientes values((select max(idPend)+1 from Pendientes), " & IdPaquete & " , '')"
                                    Consulta()
                                    i = i + 1
                                End If
                                sql = "update Pendientes set Detalle= '-" & rs2.Item(0) & " Insumo:" & rs.Item(1) & ", cantidad faltante: " & rs.Item(2) & ". ' where idPend = (select max(idPend) from Pendientes)"
                                Consulta()
                            End If
                        Else
                            eoff = True
                    ...
                    Do While (eoff = False)
                        If (rs.Read = True) Then
                            If rs.Item(2) > 0 Then
                                ...
                                rs2 = Consulta()
                                rs2.Read()
                                sql = "update Pendientes set Detalle= '-" & rs2.Item(0) & " Medicamento:" & rs.Item(1) & ", cantidad faltante: " & rs.Item(2) & ". ' where idPend = (select max(idPend) from Pendiente)"
                                Consulta()
                            End If
                        Else
                            eoff = True
